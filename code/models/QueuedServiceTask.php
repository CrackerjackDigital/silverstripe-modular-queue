...
/**
 * INCOMPLETE!!!! Like a QueuedTask but has a Service on which the method is called instead of calling it on itself. The service method is called via the 'execute'
 * dispatch interface.
 *
...
class QueuedServiceTask extends QueuedTask implements AsyncService {
	...
	public function execute( $params = [], &$resultMessage = '' ) {
		...
		$params = array_merge(
			(array) JSONData::object( $this->{JSONData::Name} ) ?: [],
			$params
		);
